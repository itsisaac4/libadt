...
Container storage and resources referenced by an element are two different kinds of ownership. This matters when a stored structure contains pointers.
...
Insertion operations copy an element's bytes into that storage. The source object itself is never retained.
...
Copying `Student_t` creates a shallow copy: it copies the pointer, not the string it points to. Both structures temporarily alias the same allocation. A configured `destroy` callback tells the container how to release resources owned by the stored element:
...
After a successful append, the stored element becomes responsible for `student.name`:
...
Do not free `student.name` after a successful append. The local structure is now a non-owning alias; ownership has transferred to the stored element.
...
The rule at the failure boundary is simple: when insertion or replacement fails, the caller still owns the source element's resources.
...
Calling `Remove` or `Discard` instead would release the element's resources and would not provide the removed value.

## Avoid Duplicate Ownership

The most common ownership bug is inserting multiple shallow copies that all claim the same resource:
...
The two stored structures would contain the same `name` pointer, causing it to be freed twice. Create a separate deep copy for each owning element.

The same rule applies when inserting an element that is already stored in a container. The byte copy is safe, but it does not copy anything referenced by the element.

`InitFrom` also performs shallow copies. If the source elements contain owned resources and the type has a destroy callback, a successful initialization makes the container responsible for those resources. The source array must then be treated as containing non-owning aliases.
...
Do not call `Init` again on a live container. It would overwrite the only references to the existing storage. Call `Destroy` first.

Calling `Destroy` repeatedly is safe after a successful initialization because the first call resets the container to zero.
